...
def updateWindow(pygame,screen,allGates,initialPos = '', pos = ''):
...
def checkAllWiring(pygame,math,screen,allGates,pos,connector):
	...
	while not finished:
		...
		updateWindow(pygame,screen,allGates,initialPos,pos)
		pygame.display.flip()
				...
				for index2,each in enumerate(allGates,start=0):
					...
					if newInOut is not None:
						if 'Inp' in newInOut and 'Out' in inOut:
							connector.append([0,index2,index])
						if 'Out' in newInOut and 'Inp' in inOut:
							connector.append([0,index,index2])
...
if __name__ == '__main__':
	global generalDic
	generalDic = {}
	generalDic['AndNum'],generalDic['OrNum'],generalDic['NotNum'] = 0,0,0
